...
class TextToSQLConverter:
    def __init__(self):
        # HuggingFace configuration (OPTIONAL - not used in pattern matching mode)
        self.hf_token = os.getenv('HUGGINGFACE_API_TOKEN')
        self.use_ai_model = False  # Set to True to use HuggingFace API
        
        # Only initialize API if token exists AND you want to use it
        if self.hf_token and self.use_ai_model:
            self.api_url = "https://api-inference.huggingface.co/models/microsoft/DialoGPT-medium"
            print("✅ HuggingFace API enabled")
        else:
            self.api_url = None
            print("ℹ️  Running in pattern-matching mode (no API calls)")
    ...
    def convert_to_sql(self, natural_query):
        ...
        NOTE: This implementation uses PATTERN MATCHING only - no API calls!
        HuggingFace token is optional and only used if use_ai_model=True
        """
        natural_query = natural_query.lower().strip()
        
        # Define SQL templates for common query patterns
        sql_templates = {
            # Sales queries
        ...
        for pattern, template_func in sql_templates.items():
            if pattern in natural_query:
                return template_func(natural_query)
        
        # If no pattern matches, try advanced pattern matching
        return self._advanced_pattern_matching(natural_query)
...
def get_query_suggestions():
    ...
    suggestions = [
        ...
        "Show sales by category this month",
